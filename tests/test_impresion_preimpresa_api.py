...
from osiris.modules.facturacion.ventas.models import Venta, VentaDetalle
...
def test_generar_plantilla_preimpresa_aplica_margenes():
    ...
    with Session(engine) as session:
        ...
        empresa = Empresa(
            razon_social="Empresa Nota Venta",
            nombre_comercial="Empresa Nota Venta",
            ruc="1790012345001",
    ...
    try:
        with TestClient(app) as client:
            response = client.get(f"/api/v1/impresion/documento/{documento_id}/preimpresa")

        assert response.status_code == 200, response.text
        assert response.headers["content-type"].startswith("text/html")
        content = response.text
        assert "padding-top: 6cm;" in content
        assert "Cantidad" in content
        assert "Descripción" in content
        assert "Servicio de mantenimiento" in content
        assert "RUC" not in content
        assert "Razón Social" not in content
        assert "Clave de Acceso" not in content
    finally:
